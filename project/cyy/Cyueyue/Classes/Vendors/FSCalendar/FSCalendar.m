...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        ...
        if (!_weekdays.count) {
            NSArray *weekSymbols = self.calendar.veryShortStandaloneWeekdaySymbols;
...
#warning 这里修改了日期的显示和颜色
//                weekdayLabel.textColor = _appearance.weekdayTextColor;
                weekdayLabel.textColor = [UIColor blackColor];
                if ([weekdayLabel.text isEqualToString:@"六"] | [weekdayLabel.text isEqualToString:@"日"]) {
                    weekdayLabel.textColor = cYellow;
                }
